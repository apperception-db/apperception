...
from Yolov5_StrongSORT_OSNet.yolov5.utils.dataloaders import VID_FORMATS, LoadImages
from Yolov5_StrongSORT_OSNet.yolov5.utils.general import (
    LOGGER, check_img_size, non_max_suppression, scale_coords, cv2, xyxy2xywh, increment_path, strip_optimizer, colorstr
)
...
def run(
                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + '.txt', 'a') as f:
                                # MOT format
                                f.write(('%g,' * 10 + f'{names[c]}\n') % (frame_idx, id, bbox_left, bbox_top, bbox_w, bbox_h, -1, -1, -1, i))
    ...
    if save_txt or save_vid:
        s = f"\n{len(list(glob.glob(save_dir + '/*.txt')))} tracks saved to {save_dir}" if save_txt else ''
