...
class View(Stage):
    ...
    def _run(self, payload: "Payload") -> "tuple[None, dict[str, list[None | list[tuple[float, float]]]]]":
        ...
        res: "list[None | list[tuple[float, float]]]" = [None] * len(payload.keep)
        for i, r in results:
            coords = r.coords
            assert len(coords) == 1
            res[i] = coords[0]

        return None, {self.classname(): res}
