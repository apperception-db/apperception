...
@pytest.mark.parametrize("fn, sql", [
    (lambda o, c: F.facingRelative(o, c.ego, c.timestamp), 
        "facingRelative(T.itemHeadings, C.egoHeading, C.timestamp)"),
    (lambda o, c: F.convert_camera(c.ego, o, c.timestamp), 
        "facingRelative(C.egoHeading, T.itemHeadings, C.timestamp)")
])

@pytest.mark.parametrize("fn, sql", [
    # (lambda o1, o2, c: F.convert_camera(o1, o2, c.timestamp), 
    #     "facingRelative(T.itemHeadings, T2.itemHeadings, C.timestamp)"),
])

def test_facingRelative(fn, sql):
