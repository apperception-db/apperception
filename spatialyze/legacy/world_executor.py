...
from spatialyze.legacy.metadata_context import geometry, primarykey, time
from spatialyze.legacy.metadata_context_executor import MetadataContextExecutor
from spatialyze.legacy.video_context_executor import VideoContextExecutor
from spatialyze.utils import (
...
class WorldExecutor:
    ...
    def get_video(self, metadata_results):
        ...
        for i in range(len(cam_nodes)):
            ...
            for item_id, vals in metadata_results.items():
                ...
                vid_fname = (
                    "./output/"
                    + self.curr_world.VideoContext.camera_nodes[cam_id].metadata_id
                    + item_id
                    + ".mp4"
                )
                # print(vid_fname)
    ...
    def execute(self):
        ...
        if self.curr_world.MetadataContext.scan.view is None:
            return

        if self.curr_world.GetVideo:
            if self.tasm:
                metadata_executor = MetadataContextExecutor(
                    self.conn, self.curr_world.MetadataContext.get_columns(primarykey, time)
                )
                ...
                metadata_executor = MetadataContextExecutor(
                    self.conn,
                    self.curr_world.MetadataContext.get_columns(primarykey, geometry, time),
                )
