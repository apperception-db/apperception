...
def ego_stop(ego_trajectory: "List[trajectory_3d]", ego_config: "CameraConfig"):
    ...
    if _ego_stop:
        action = Action(
            current_time, ego_departure_time, ego_loc, ego_departure_loc, action_type=EGO_STOP
        )
...
def ego_exit_current_segment(
    ...
    exit_time, exit_point = time_to_exit_current_segment(
        detection_info, current_time, ego_loc, car_trajectory=ego_trajectory, is_ego=True
    )
...
def car_meet_up_with_ego(
    ...
    meet_up_time, meetup_point = meetup(
        ego_loc, car2_loc, car1_heading, car2_heading, road_type, current_time, car1_trajectory
    )
    if meet_up_time < current_time:
        return None
    meet_up_action = Action(
        current_time,
        meet_up_time,
        start_loc=car2_loc,
        end_loc=meetup_point,
        action_type=MEET_UP,
        target_obj_id=detection_info.detection_id,
    )
...
def car_exit_view(
    ...
    exit_view_action = Action(
        current_time,
        exit_view_time,
        start_loc=car_loc,
        end_loc=exit_view_point,
        action_type=EXIT_VIEW,
        target_obj_id=detection_info.detection_id,
    )
...
def same_direction_sample_action(detection_info: "DetectionInfo", view_distance: float):
    ...
    return combine_sample_actions(
        [ego_exit_segment_action, car_exit_segment_action, car_go_beyong_view_action]
    )
...
def get_sample_action_alg(
    relative_direction: "Union[None, Literal['same_direction', 'opposite_direction']]",
):
