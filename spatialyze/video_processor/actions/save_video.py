...
def save_video(payload: "Payload", filename: str, bbox: bool = True, depth: bool = True) -> None:
    ...
    for frame, k, tracking, tracking3d in tqdm(
        zip(iterate_video(video), payload.keep, trackings, trackings3d),
        total=len(payload.keep),
    ):
        if not k:
            frame[:, :, 2] = 255
        ...
        if bbox and payload.metadata is not None:
            if tracking is not None:
                ...
                for id, t in tracking.items():
                    ...
                    id = int(id)
                    label = (
                        f"{id} {c} conf: {t.confidence:.2f} dist: {t3d.point_from_camera[2]: .4f}"
                    )
                    annotator.box_label(
                        [
                            t.bbox_left,
