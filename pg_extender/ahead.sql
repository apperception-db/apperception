...
$BODY$
BEGIN
  RETURN (ST_X(obj1_loc) - ST_X(obj2_loc)) * COS(PI() * (obj2_heading + 90) / 180) + (ST_Y(obj1_loc) - ST_Y(obj2_loc)) * SIN(PI() * (obj2_heading + 90) / 180) > 0 AND
         ABS(ST_X(convertCamera(obj1_loc, obj2_loc, obj2_heading))) < 1;
END
$BODY$
LANGUAGE 'plpgsql' ;
