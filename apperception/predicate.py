...
class PredicateNode:
    def __init__(self, *args, **kwargs):
        ...
        if len(args) + len(kwargs) != len(anns):
            raise Exception(f"Mismatch number of arguments: expecting {len(anns)}, received {len(args)} args and {len(kwargs)} kwargs")
...
class GenSqlVisitor(Visitor[str]):
    def visit_ArrayNode(self, node: "ArrayNode"):
        elts = ",".join(
            self.visit(e)[5 if isinstance(e, ArrayNode) else 0 :] for e in node.exprs
        )
        return f"ARRAY[{elts}]"
    ...
    def visit_BinOpNode(self, node: "BinOpNode"):
        ...
        if isinstance(node.left, ArrayNode):
            return self.visit(ArrayNode([
                BinOpNode(l, node.op, node.right)
                for l in node.left.exprs
            ]))
    ...
    def visit_LiteralNode(self, node: "LiteralNode"):
        value = node.value
        if isinstance(value, str) and node.python == True:
            return f'"{value}"'
        else:
            return str(value)
