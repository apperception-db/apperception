...
CREATE_MAIN_BBOXES_SQL ='''CREATE TABLE IF NOT EXISTS Main_Bbox(
	itemId TEXT,
	cameraId TEXT,
	trajBbox stbox,
...
CREATE_TEMP_BBOXES_SQL ='''CREATE TABLE IF NOT EXISTS Temp_Bbox(
	itemId TEXT,
	cameraId TEXT,
	trajBbox stbox
	);'''
...
def add_scenic_recognized_objs(conn, cam_id, formatted_result, start_time, default_depth=True):
	...
	for item_id in formatted_result:
		...
		centroids = np.mean(recognized_bboxes, axis=1)
		...
		scenic_bboxes_to_postgres(conn, cam_id, item_id, object_type, "default_color", start_time, tracked_cnt, obj_traj, centroids)
		# bbox_to_tasm()


# Insert bboxes to postgres
def scenic_bboxes_to_postgres(conn, cam_id, item_id, object_type, color, start_time, timestamps, bboxes, centroids):
	postgres_timestamps = convert_timestamps(start_time, timestamps)
	create_or_insert_trajectory(conn, item_id, cam_id, object_type, color, postgres_timestamps, bboxes, centroids)
	# print(f"{item_id} saved successfully")


def create_or_insert_trajectory(conn, item_id, camera_id, object_type, color, postgres_timestamps, bboxes, centroids, reconcile_on_id=True):
	'''
	...
	reconcile_trajectory(conn, reconcile_on_id)
...
def create_main_trajectory_and_main_bbox(conn):
	...
	cursor.execute(CREATE_MAIN_BBOXES_SQL)
...
def create_or_insert_temp_trajectory(conn, item_id, camera_id, object_type, color, postgres_timestamps, bboxes, centroids):
	...
	cursor.execute(CREATE_TEMP_BBOXES_SQL)
...
def reconcile_trajectory(conn, reconcile_on_id=True, threshold=100):
	cursor = conn.cursor()
	cursor.execute(f"SELECT reconcileTrajectory({reconcile_on_id}, {threshold});")
	conn.commit()
	# pass
