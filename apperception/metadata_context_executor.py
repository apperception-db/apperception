...
class MetadataContextExecutor:
    ...
    def visit(self, create_view, view_name):
        ...
        if create_view:
            db_query = "CREATE VIEW "+view_name+" AS "+select_query + from_query + where_query + ";"
            print(db_query + "\n")
        ...
        if scan_node.view:
            if scan_node.view.default:
                if scan_node.view == metadata_view:
                    from_query += metadata_view.trajectory_view.view_name + " INNER JOIN " + metadata_view.location_view.view_name \
                        + " USING(itemId) "
                else:
