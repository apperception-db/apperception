...
def scenic_generate_df():
    ...
    df_sample_annotation["heading"] = df_sample_annotation.apply(lambda x: get_heading(x.rotation) % 360, axis=1)
    df_sample_data["heading"] = df_sample_data.apply(lambda x: (get_heading(x.camera_rotation) + get_heading(x.ego_rotation)) % 360, axis=1)
