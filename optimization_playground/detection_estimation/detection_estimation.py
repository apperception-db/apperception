...
#TODO
class samplePlan:
    def __init__(self, video, all_detection_info=None, metadata=None):
        self.video = video
        self.action = None
    ...
    def add(self, priority, sample_action, time_threshold = 0.5):
        if self.current_priority is None:
            ...
            if sample_action.estimated_tune < self.action.estimated_tims:
    ...
    def get_next_frame_num(self, current_frame_num):
        next_frame_name, next_frame_num, next_frame_time = self.get_next_frame_info()
        return max(next_frame_num, current_frame_num + 1)
...
def detection_estimation(sorted_ego_config, video, start_frame_num, metadata, view_distance=50):
    ...
    while current_frame:
        if next_frame_config.get_frame_num() == next_frame_num:
            ...
            next_sample_plan = generate_sample_plan(video, all_detection_info, next_sample_plan, metadata)
